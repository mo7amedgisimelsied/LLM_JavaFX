...
public class Icons {
    ...
    public static SVGPath getRectangleIcon() {
        // Scaled down or viewed in a specific box. The provided path is large (640), scaling it down via scale properties in CSS or Code is usually needed.
        // We will scale path data or use SetScale in usage. Here we return raw data.
        SVGPath path = createPath("M512 112C520.8 112 528 119.2 528 128L528 512C528 520.8 520.8 528 512 528L128 528C119.2 528 112 520.8 112 512L112 128C112 119.2 119.2 112 128 112L512 112zM128 64C92.7 64 64 92.7 64 128L64 512C64 547.3 92.7 576 128 576L512 576C547.3 576 576 547.3 576 512L576 128C576 92.7 547.3 64 512 64L128 64z");
        path.setScaleX(0.04); // Rough normalization for 640px icon to ~24px
        path.setScaleY(0.04);
        return path;
    }

    public static SVGPath getCircleIcon() {
        SVGPath path = createPath("M528 320C528 205.1 434.9 112 320 112C205.1 112 112 205.1 112 320C112 434.9 205.1 528 320 528C434.9 528 528 434.9 528 320zM64 320C64 178.6 178.6 64 320 64C461.4 64 576 178.6 576 320C576 461.4 461.4 576 320 576C178.6 576 64 461.4 64 320z");
        path.setScaleX(0.04);
        path.setScaleY(0.04);
        return path;
    }
    ...
    public static SVGPath getTriangleIcon() {
        ...
        path.setScaleX(1.4); // Scale up slightly to match 24px box
        path.setScaleY(1.4);
        return path;
    }
    ...
    public static SVGPath getTextIcon() {
        SVGPath path = createPath("M12.258 3h-8.51l-.083 2.46h.479c.26-1.544.758-1.783 2.693-1.845l.424-.013v7.827c0 .663-.144.82-1.3.923v.52h4.082v-.52c-1.162-.103-1.306-.26-1.306-.923V3.602l.431.013c1.934.062 2.434.301 2.693 1.846h.479z");
        path.setScaleX(1.4);
        path.setScaleY(1.4);
        return path;
    }
